...
class TestGQLDynamicQueryBuilderNestedFields:
    ...
    def test_build_with_api_where_clause_one_op_one_value_nested_field(self, query, subquery_to_test):
    ...
    @pytest.mark.parametrize("query,subquery_to_test", ALL_QUERIES)
    def test_build_with_api_where_clause_one_op_many_values_nested_field(self, query, subquery_to_test):
        builder = GQLDynamicQueryBuilder(query)
    ...
    @pytest.mark.parametrize("query,subquery_to_test", ALL_QUERIES)
    def test_build_with_api_where_clause_many_ops_one_value_nested_field(self, query,subquery_to_test):
        builder = GQLDynamicQueryBuilder(query)

        with pytest.raises(TypeError):
            builder.with_where_clause('subquery_to_test', 'subquery_to_test_body_arg.name', 'test', ['_in', '_eq'])


    @pytest.mark.parametrize("query,subquery_to_test", ALL_QUERIES)
    def test_build_with_api_where_clause_many_ops_many_values_nested_field(self, query,subquery_to_test):
        builder = GQLDynamicQueryBuilder(query)
        builder.with_where_clause('subquery_to_test', 'subquery_to_test_body_arg.name', ['test', 'hallo'], ['_gte', '_lt'])
        result = builder.build()
