...
class TestGQLDynamicQueryBuilderSimpleFields:
    ...
    @pytest.mark.parametrize('query,subquery_to_test', ALL_QUERIES)
    def test_build_with_api_where_clause_many_ops_one_value(
        self, query, subquery_to_test
    ):
        builder = GQLDynamicQueryBuilder(query)
        ...
        with pytest.raises(TypeError):
            builder.with_where_clause(
                'subquery_to_test', 'subquery_to_test_body_arg', 'test', ['_in', '_eq']
            )
