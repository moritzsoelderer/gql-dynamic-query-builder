from pyparsing import Literal, Forward, QuotedString, Word, alphas, ZeroOrMore, ParserElement
...
WHERE_CLAUSE = Literal('where') + Literal(':') + Literal('{') + ZeroOrMore(RECURSIVE_WHERE_CONDITION) + Literal('}')
...
def get_new_where_clause_and_content() -> tuple[ParserElement, ParserElement]:
    ...
    where_clause = Literal('where') + Literal(':') + Literal('{') + recursive_where_condition + Literal('}')
